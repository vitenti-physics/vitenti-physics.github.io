---
title: "Aula 8: Funções de Green em 2D e ND"
...
3. **Generaliza** para espaços curvos (relatividade geral)
...
$$
G_N \propto \sigma^{1-N/2} = |\mathbf{x} - \mathbf{x}_0|^{2-N}
$$
...
$$
G_3 \propto \sigma^{-1/2} = \frac{1}{|\mathbf{x} - \mathbf{x}_0|}
$$
...
$$
I_\varepsilon = -3\varepsilon \int_0^{2\pi} \int_0^\pi \int_0^\infty \frac{r^2 \sin\theta}{(r^2/2 + \varepsilon)^{5/2}} \, dr \, d\theta \, d\phi
$$

2. **Integração angular**:

$$
I_\varepsilon = -4\pi \cdot 3\varepsilon \int_0^\infty \frac{r^2}{(r^2/2 + \varepsilon)^{5/2}} \, dr
$$

3. **Integração por partes** (para simplificar):

$$
I_\varepsilon = -4\pi \cdot 3\varepsilon \left[ \frac{r}{(r^2/2 + \varepsilon)^{3/2}} \right]_0^\infty - \int_0^\infty \frac{1}{(r^2/2 + \varepsilon)^{3/2}} \, dr
$$

4. **Substituição** $r = \sqrt{2\varepsilon}\tan\theta$:

$$
\int_0^\infty \frac{dr}{(r^2/2 + \varepsilon)^{3/2}} = \frac{1}{\varepsilon}\int_0^{\pi/2} \cos\theta \, d\theta = \frac{1}{\varepsilon}
$$

5. **Resultado final**:

$$
I_\varepsilon = -4\pi \cdot 3\varepsilon \cdot \frac{1}{\varepsilon} \cdot (-1) = -4\pi
$$
...
$$
G_3(\mathbf{r} - \mathbf{r}_0) = -\frac{1}{4\pi|\mathbf{r} - \mathbf{r}_0|}
$$
...
$$
\lim_{\varepsilon \to 0} \int \nabla^2 \left(-\frac{1}{4\pi\sqrt{\sigma+\varepsilon}}\right) f(\mathbf{x}) \, d^3x = f(\mathbf{x}_0)
$$
...
Medidas precisas da lei de Coulomb podem revelar a dimensão e topologia do espaço:

- **Dimensão exata**: Se a força variasse como $1/r^{2+\epsilon}$, indicaria dimensões fracionárias ou dimensões extras
- **Topologia não-trivial**: Em universos com topologia de toro, poderíamos observar múltiplas imagens de uma mesma fonte

## Teoria das Cordas

Em teoria de cordas, dimensões extras podem ser "compactificadas" (enroladas em escalas muito pequenas). Nesse caso:

1. Em distâncias grandes (> escala de compactificação): 3D efetivo
2. Em distâncias pequenas (< escala de compactificação): $N$D efetivo

A lei de Coulomb seria modificada em distâncias muito pequenas, fornecendo um teste experimental para dimensões extras.

::: {.callout-note}
A regularização dimensional utilizada em física de partículas é análoga à técnica de Frobenius para raízes repetidas. Em ambos os casos, tratamos dimensões não-inteiras como um artifício matemático para obter resultados finitos.
:::
...
## Exercício 8: Topologia do Universo

**a)** O que são "círculos no céu" e como se relacionam com a topologia do universo?

**b)** Por que a topologia do universo é difícil de determinar experimentalmente?

**c)** Qual é a relação entre topologia e funções de Green?

**d)** Como a função de Green mudaria em um universo com topologia de toro?

## Exercício 9: Regularização Dimensional

**a)** Explique o que é regularização dimensional em física de partículas.

**b)** Como ela se relaciona com o método de Frobenius usado na aula?

**c)** Por que a regularização dimensional funciona mesmo com dimensões não-inteiras?

**d)** Dê um exemplo de como a regularização dimensional é usada para calcular integrais divergentes.

## Exercício 10: Implementação Numérica
...
## Exercício 11: Conexão com Eletromagnetismo
...
## Exercício 12: Caso 1D Detalhado
...
## Exercício 13: Método de Frobenius
...
## Exercício 14: Espaços Curvos

**a)** Como a função de Synge se generaliza para espaços curvos?

**b)** Qual é a equação que a função de Green satisfaz em um espaço curvo?

**c)** Como a curvatura afeta a função de Green?

**d)** Por que a função de Synge é importante em relatividade geral?

## Exercício 15: Aplicações Práticas
