...
field directly from Maxwell's equations. In Heaviside–Lorentz units with $c=1$, the
equations containing the sources are
...
=\mathbf E\cdot(\nabla\times\mathbf B)-\mathbf E\cdot\partial_t\mathbf E.
$$
...
$$
\mathbf J\cdot\mathbf E
=-\partial_t\!\left[\tfrac12\left(E^2+B^2\right)\right]
-\nabla\cdot(\mathbf E\times\mathbf B).
$$
...
$\sigma^{ij}=-T^{ij}=E^iE^j+B^iB^j-\tfrac12\delta^{ij}(E^2+B^2)$, this becomes
